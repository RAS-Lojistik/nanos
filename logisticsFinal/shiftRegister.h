...
public:
  uint8_t byte;
  SR(const uint8_t dataPin, const uint8_t clockPin, const uint8_t strobePin):
    _dataPin(dataPin), 
    _clockPin(clockPin), 
  ...
  inline void setBit(const uint8_t bitOrder, const uint8_t state) {
    if(((1UL << bitOrder) & this->byte) ^ state) {
      this->byte ^= (1UL << bitOrder);
      this->push();
    }
  } 
  
  inline void setBits(const uint8_t bitMask, const uint8_t byte) {
    uint8_t tempByte = this->byte;
    for (uint8_t i = 0; i < 8; i++) {
      if((1UL << i) & bitMask) {
        if(((1UL << i) & tempByte) ^ ((1UL << i) & byte)) {
          tempByte ^= (1UL << i);
        }
      }
    }
